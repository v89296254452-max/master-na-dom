...
function corePages() {
  ...
  const tierRank = { "миллионник": 0, "крупный": 1, "средний": 2, "малый": 3 };
  const pages = rec.slice(1).filter(r => {
    if (!r[iS]) return false;
    const f = facts.get((r[iC] || "").trim().toLowerCase());
    return f && f.districts && f.districts.length; // только города с реальными районами
  }).map(r => ({
    slug: r[iS].trim(), city: (r[iC] || "").trim(), cityPrep: (r[iP] || r[iC] || "").trim(), service: (r[iSv] || "").trim(),
  }));
  // приоритет: крупные города первыми
  pages.sort((a, b) => (tierRank[facts.get(a.city.toLowerCase())?.population_tier] ?? 2) - (tierRank[facts.get(b.city.toLowerCase())?.population_tier] ?? 2));
  return pages;
}
