...
import { contentPackInputFromTask, generateVkVisualPrompts } from "@/lib/vk-content-pack-server";
...
export async function POST(request: Request) {
  try {
    ...
    const visuals = generateVkVisualPrompts(contentPackInputFromTask(existing));
    const contentPack = {
      ...existing.contentPack,
      avatarPrompt: visuals.avatarPrompt,
      coverPrompt: visuals.coverPrompt,
    };
    ...
    appendVkTaskLogEntry({
      ...
      message: "Визуалы перегенерированы",
    });

    return NextResponse.json({ success: true, task: updated });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Не удалось перегенерировать визуалы";
