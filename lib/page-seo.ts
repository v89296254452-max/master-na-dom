...
const UNIQUE_EXTRA_PARAGRAPHS: ExtraBuilder[] = [
  (c) =>
    `Служба «${BRAND}» работает в ${c.cityPrep} и принимает звонки круглосуточно — 24/7, без выходных. Мастер по услуге ${c.serviceQ} выезжает в течение 30–60 минут — в центр, спальные районы и пригород. Диагностика бесплатна при выполнении работ, стоимость согласовывается до начала ремонта.`,
  (c) =>
    `В ${c.cityPrep} часто заказывают услугу ${c.serviceQ} при первых признаках неисправности: это помогает избежать дорогостоящего ремонта и простоя техники. Наши специалисты приезжают с инструментом и типовыми запчастями, большинство проблем устраняют за один визит.`,
  (c) =>
    `Обслуживаем все районы в ${c.cityPrep}: ${c.districts || "весь город"}. Мастер заранее согласует время приезда и перезванивает за 15–20 минут до выезда. После ремонта вы получаете гарантию на работы — от 6 до 12 месяцев в зависимости от вида услуги.`,
  (c) =>
  ...
  (c) =>
    `Типовой выезд по услуге ${c.serviceQ} в ${c.cityPrep} занимает 1–2 часа. Мастер проводит диагностику, называет точную цену и приступает к работе только после вашего согласия. Оплата — наличными или переводом после выполнения.`,
  (c) =>
  ...
  (c) =>
    `Наши мастера знают особенности жилого фонда в ${c.cityPrep}: старые дома с изношенными коммуникациями, новостройки с современной разводкой, частный сектор. Это ускоряет диагностику и помогает сразу предложить правильное решение.`,
  (c) =>
    `Заказать услугу ${c.serviceQ} в ${c.cityPrep} можно по телефону ${c.phone} или через форму на сайте. Укажите адрес, опишите проблему — диспетчер перезвонит в течение 5 минут и уточнит детали заявки.`,
  (c) =>
  ...
  (c) =>
    `Сезонные нагрузки в ${c.cityPrep} влияют на частоту обращений: летом растёт спрос на ремонт кондиционеров и холодильников, зимой — на отопление и электрику. Мы держим резерв мастеров, чтобы выезжать без длительного ожидания.`,
  (c) =>
  ...
  (c) =>
    `После завершения работ мастер убирает рабочую зону и проверяет результат вместе с вами. На все работы по услуге ${c.serviceQ} в ${c.cityPrep} действует гарантия — сохраните контакт ${c.phone} для быстрой связи при необходимости.`,
  (c) =>
    `Сравнивая предложения в ${c.cityPrep}, обращайте внимание не только на цену, но и на гарантию, скорость выезда и отзывы. Мы работаем официально, выдаём документы по запросу и фиксируем согласованную стоимость до начала ремонта.`,
];
...
export function getPageSeoSections(page: Page): PageSeoSections {
  ...
  // Приоритет — уникальный ИИ-текст (если сгенерирован для этого slug),
  // иначе шаблонная перестановка абзацев.
  const ai = getAiContent(page.slug);

  return {
    uniqueText: {
      title: blocks[0]?.title ?? `${page.service || "Услуга"} в ${cityPrep}`,
      paragraphs: ai ? ai.paragraphs : buildUniqueParagraphs(page, blocks),
    },
